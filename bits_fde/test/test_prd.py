...
def test_global_test():
    ...
    # Global test
    raw_pd = test.global_test(raw_pd, sigma=sigma, alpha=0.05, number_of_unknown=4, weight_column="weight",
    ...
    cov = raw_pd[["cov_bx_rx_m", "cov_by_rx_m", "cov_bz_rx_m"]]
    d_major = np.sqrt(cov.sum(axis=1))
    raw_pd["uncertainty_m"] = d_major
    raw_pd = hpl.hpl(raw_pd, sigma=sigma, alpha=0.05, dof=4, uncertainty_column= "uncertainty_m",
    ...
    assert (len(baseline_pd) > 0) and (baseline_pd["error_m"] - gt_uncertainty < baseline_pd["hpl_m"]).all(), txt
