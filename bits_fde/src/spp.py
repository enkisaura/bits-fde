...
def global_test(pd_gnss_raw: pd.DataFrame, alpha:float=0.05, sigma:float|None=None, pd_ephemeris: pd.DataFrame = None,
    ...
    for timestamp, group in tqdm(pd_gnss_raw.groupby("time", sort=True), desc="Applying global test"):
        ...
        result, chi2_stat, chi2_threshold = test.window_global_test(residuals, W, alpha, number_of_unknown)

        group["valid_estimate"] = result
        group["test_statistic"] = float(chi2_stat.item())
        group["test_threshold"] = float(chi2_stat.item())
